...
A concept emerged from the idea that these 'hidden conversations' need not be lost to time. A small, portable micropohone, speaker, and small battery-powered computing device could allow people to leave messages behind that can be experienced after-the-fact by those on the opposite wall.
...
The computing device is Rasberry Pi. Javier was learning the tech stack from scratch, and had a lot of ground to cover, but fortunately the Pi is well-supported.

The team had to consider problems of power. Which speakers can be powered at what volume and for how long?
